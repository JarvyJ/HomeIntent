...
<script>
  ...
  function setupSocket(name, url) {
    ...
    sockets[name].socket.onclose = function(event) {
      if (event.wasClean) {
        sockets[name].status = `Connection closed cleanly, code=${event.code} reason=${event.reason}`
      } else {
...
<div class:grid-cols-2={!horizontal_split} class:grid-cols-1={horizontal_split} class="grid justify-items-stretch w-full h-screen gap-5 p-5">
  ...
  <div class="justify-self-stretch pb-9">
    ...
    <textarea readonly class:dont-word-wrap={!word_wrap} bind:this={socketElements[name]} class="w-full h-full p-2 bg-gray-300 rounded-md font-mono text-sm">{info.messages.join("\n")}</textarea>
  </div>
  {/each}
</div>
