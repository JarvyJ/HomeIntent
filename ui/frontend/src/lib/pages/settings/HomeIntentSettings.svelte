<script>
	...
	const dropdownStyle =
		'border chevron-down p-1.5 pr-12 dark:border-gray-500 rounded-md appearance-none w-full dark:bg-gray-800';
	...
	async function testMicrophones() {
		...
		const response = await fetch(`/api/v1/rhasspy/audio/test-microphones`);
		if (response.ok) {
	...
	<div class="text-lg grid grid-cols-4 w-1/2 items-center gap-x-5 gap-y-6">
		{#each Object.entries(effects) as [name, meta] (name)}
			<span
				...
				on:click={() => playSoundEffect(name)}><PlayCircleFill /></span
			>
			<div>
			...
			<label class="ml-auto rounded hover:bg-green-200 bg-hi-green px-3 py-1 cursor-pointer">
				<span>Upload</span>
				<input type="file" {name} class="hidden" accept=".wav" on:change={uploadSoundEffect} />
			</label>
