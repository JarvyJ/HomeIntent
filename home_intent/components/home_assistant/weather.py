...
class Weather:
    def __init__(self, home_assistant):
        ...
        self.entity_with_forecast = [x for x in self.entities if x["attribute"].get("forecast") is not None]
    ...
    @intents.sentences([
        "what is the temperature (right now|today|outside)"
    ])
    def weather_day(self, day_of_week):
    ...
    def forecast_today(self, day_of_week):
    ...
    def forecast_tonight(self, day_of_week):
        response = self.ha.api.get_entity(self.entity_with_forecast)
...
def _get_forecast(forecast):
    if current_conditions['detailed_description']:
        return current_conditions['detailed_description']
